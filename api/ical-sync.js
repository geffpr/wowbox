  ...
  try {
    ...
    for (const block of eventBlocks) {
      ...
      if (rruleMatch) {
        ...
        // If multi-day event, block all days
    ...
    return res.json({ success: true, count: upserted, total_events: eventBlocks.length, total_dates: totalDates });
...
// ── RRULE expander ────────────────────────────────────────────────────────────
function expandRRule(dtStart, rruleStr, horizon, today, exdates) {
  const rule     = parseRRule(rruleStr);
  const freq     = rule.FREQ     || 'WEEKLY';
  const interval = rule.INTERVAL || 1;
  const maxCount = rule.COUNT    || 9999;
  const until    = rule.UNTIL    ? (rule.UNTIL < horizon ? rule.UNTIL : horizon) : horizon;
  ...
  while (cur <= until && count < maxCount && iter < maxIter) {
    iter++;
    if (freq === 'DAILY') {
      if (cur >= today) { const k = cur.toISOString().slice(0,10); if (!exdates.has(k)) { results.push(k); count++; } }
      ...
      if (byDay.length > 0) {
        ...
        byDay.forEach(dayName => {
          ...
          if (dayDate >= today && dayDate >= cur && dayDate <= until && !exdates.has(k)) {
      ...
      if (cur >= today) { const k = cur.toISOString().slice(0,10); if (!exdates.has(k)) results.push(k); }
      break;
    }
  }
  return results;
...
function parseRRule(str) {
  ...
  str.split(';').forEach(part => {
    ...
    if (!k || !v) return;
    if (k === 'FREQ')     rule.FREQ     = v.trim();
    if (k === 'INTERVAL') rule.INTERVAL = parseInt(v);
    if (k === 'COUNT')    rule.COUNT    = parseInt(v);
    if (k === 'BYDAY')    rule.BYDAY    = v.split(',').map(s => s.trim());
    if (k === 'UNTIL')    rule.UNTIL    = parseIcsDate(v.replace('Z','').trim());
  });
  return rule;
}

// Parse only the DATE part — ignore time — returns midnight UTC
